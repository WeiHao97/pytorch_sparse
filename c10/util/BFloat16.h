...
namespace detail {
  ...
  inline float fp32_from_bits2(uint32_t w) {
    ...
    std::cout << "weird place1" << std::endl;
    union {
      uint32_t as_bits;
      float as_value;
    } fp32 = {w};
  ...
  inline uint32_t fp32_to_bits2(float f) {
  #if defined(__OPENCL_VERSION__)
    return as_uint(f);
  #elif defined(__CUDA_ARCH__)
  ...
  #else
    std::cout << "weird place2" << std::endl;
    union {
      float as_value;
      uint32_t as_bits;
    } fp32 = {f};
   ...
   */
  inline uint32_t fp16_ieee_to_fp32_bits2(uint16_t h) {
          /*
   ...
   */
  inline float fp16_ieee_to_fp32_value2(uint16_t h) {
          /*
           * Extend the half-precision floating-point number to 32 bits and shift to the upper part of the 32-bit word:
           *      +---+-----+------------+-------------------+
           *      | S |EEEEE|MM MMMM MMMM|0000 0000 0000 0000|
    ...
    const float normalized_value = fp32_from_bits2((two_w >> 4) + exp_offset) * exp_scale;
          ...
          const float denormalized_value = fp32_from_bits2((two_w >> 17) | magic_mask) - magic_bias;
          ...
          const uint32_t result = sign |
                  (two_w < denormalized_cutoff ? fp32_to_bits2(denormalized_value) : fp32_to_bits2(normalized_value));
          return fp32_from_bits2(result);
  }
   ...
   * floating-point operations and bitcasts between integer and floating-point variables.
   */
  inline uint16_t fp16_ieee_from_fp32_value2(float f) {
    ...
    const float scale_to_zero = scale_to_zero_val;
          ...
          const uint32_t w = fp32_to_bits2(f);
          ...
          base = fp32_from_bits2((bias >> 1) + UINT32_C(0x07800000)) + base;
          const uint32_t bits = fp32_to_bits2(base);
